...
public class LRUMap<T> extends LinkedHashMap<String, SoftReference<T>> implements Externalizable {
    ...
    /**
     * 让LinkHashMap支持LRU，如果Map的大小超过了预定值，则返回true
     */
    @Override
    ...
    /**
     * Adds a new entry into this cache, checking the size and discarding the
     * least recently accessed entry if the size is equal to the maximum cache
     * size.
     * 
     * @param int key
     * @param entry
    ...
    public T addEntry(String key, T entry) {
        try {
            ...
            // if (containsKey(key)) return entry;
            // add the value as a SoftReference
